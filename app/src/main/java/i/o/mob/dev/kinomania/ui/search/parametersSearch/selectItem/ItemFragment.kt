package i.o.mob.dev.kinomania.ui.search.parametersSearch.selectItem
...
class ItemFragment : Fragment(R.layout.fragment_item_list), SimpleItemDelegate {
    ...
    override fun onViewCreated(view: View, savedInstanceState: Bundle?) {
        ...
        choose.setOnClickListener {
            when(currentList){
                LIST_TYPE -> {
                    findNavController().previousBackStackEntry?.savedStateHandle?.set("key", LIST_TYPE to selectedItemsMap)
                    requireActivity().onBackPressed()
                }
                LIST_GENRE -> {
                    findNavController().previousBackStackEntry?.savedStateHandle?.set("key", LIST_GENRE to selectedItemsMap)
                    requireActivity().onBackPressed()
                }
                LIST_COUNTRY -> {
                    findNavController().previousBackStackEntry?.savedStateHandle?.set("key", LIST_COUNTRY to selectedItemsMap)
                    requireActivity().onBackPressed()
                }
            }
        }
